...
# Gatilho de limpeza para o Calibre
if uploaded_calibre is not None:
    if "last_calibre" not in st.session_state or st.session_state["last_calibre"] != uploaded_calibre.name:
        ...
        if "df_calibre_proc" in st.session_state:
            del st.session_state["df_calibre_proc"]
...
# Mapeamento Dinâmico de Studs Ausentes (EXCLUSIVO POR PÁ)
st.sidebar.markdown("---")
...
def run_analysis(df_in: pd.DataFrame, full_process=False, t_sel=None, b_sel=None, i_sel=None):
    ...
    for keys, g in long_df.groupby(["Turbina", "Blade", "Casca", "Regiao", "Relogio", "Inspecao"]):
        ...
        if not full_process:
            verify_items.append({"turbina": turb, "blade": blade, "casca": casca, "regiao": reg, "relogio": relogio, "inspecao": insp, "data": g.sort_values("Ponto")[["Ponto", "Valor_mm"]], "delta": d})
    ...
    for blade in _blades_sel:
        ...
        for (casca, reg, relogio), g_sensor in df_b.groupby(["Casca", "Regiao", "Relogio"]):
            ...
            for (tb, isp), g_trace in g_sensor.groupby(["Turbina", "Inspecao"]):
                ...
                val_d = delta_summary[(delta_summary["Turbina"].astype(str) == str(tb)) & (delta_summary["Blade"].astype(str) == str(blade)) & (delta_summary["Casca"].astype(str) == str(casca)) & (delta_summary["Regiao"].astype(str) == str(reg)) & (delta_summary["Relogio"].astype(str) == str(relogio)) & (delta_summary["Inspecao"].astype(str) == str(isp))]["Delta_medio_ciclo_mm"].max()
    ...
    return {
        ...
        "delta_summary": delta_summary, "severity_by_blade_latest": blade_latest, "severity_by_turbine_latest": turbine_latest,
        "reinspection_table": reinspection, "critical_blades": critical_blades, "critical_turbines": critical_turbines,
        "pdf_detailed_data": pdf_detailed_data, "verify_data": verify_items, "latest_sensors": latest_sensors 
    }

# ---------------------------------------------------------------------
# Helpers de Gráficos e PDFs
# ---------------------------------------------------------------------
def create_radar_chart(blade, latest_sensors, studs_ausentes_dict, engine='plotly'):
    ...
    if engine == 'plotly':
        fig = go.Figure()
        fig.add_trace(go.Scatterpolar(r=[1.1, 0, 1.1], theta=[0, 0, 180], mode='lines', line=dict(color='black', width=1.5, dash='dash'), hoverinfo='none', showlegend=False))
        for f in range(84):
            ang = calculate_angle(f)
            fig.add_trace(go.Scatterpolar(r=[0.9, 1.1], theta=[ang, ang], mode='lines', line=dict(color='grey', width=0.5), showlegend=False, hoverinfo='none'))
        ...
        for sens_key in MAPA_FUROS.keys():
            ...
            fig.add_trace(go.Scatterpolar(
                ...
                hoverinfo='text', hovertext=f"<b>{sens_key}</b><br>Gap: {gap:.1f}mm<br>Sev: {sev}", name=sens_key
            ))
        ...
        for stud in studs_ausentes:
            ...
            fig.add_trace(go.Scatterpolar(
                ...
                hoverinfo='text', hovertext=f"Stud {stud} Ausente (Zona {get_stud_zone(stud)})", name='Stud Ausente', showlegend=False
            ))

        fig.update_layout(
            polar=dict(angularaxis=dict(direction="counterclockwise", rotation=0, showticklabels=False), radialaxis=dict(showticklabels=False, range=[0, 1.3])),
            showlegend=False, height=450, margin=dict(l=30, r=30, t=30, b=30), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)'
        )
    ...
    for sens in sorted(df_b["Sensor"].unique()):
        ...
        if g_trim is not None and not g_trim.empty:
            if sens.startswith("PS"):
                ...
                ss_idx += 1
    ax_ps.set_title(f"PS Curves - {data_str}", fontsize=9, fontweight='bold', pad=8); ax_ps.grid(True, linestyle='--', alpha=0.5); ax_ps.legend(fontsize=7, loc='upper right')
    ax_ss.set_title(f"SS Curves - {data_str}", fontsize=9, fontweight='bold', pad=8); ax_ss.grid(True, linestyle='--', alpha=0.5); ax_ss.legend(fontsize=7, loc='upper right')
    plt.tight_layout(); img_io = io.BytesIO(); fig.savefig(img_io, format='png', dpi=150, bbox_inches='tight'); img_io.seek(0); plt.close(fig)
    return img_io

def create_radar_chart_and_table(blade, latest_sensors, studs_ausentes_dict):
    ...
    img_io_polar = io.BytesIO(); fig_polar.savefig(img_io_polar, format='png', dpi=150, bbox_inches='tight', transparent=True); img_io_polar.seek(0); plt.close(fig_polar)
    return img_io_polar, table_data

def get_proportional_image(img_bytes, max_w, max_h):
    try:
        img_bytes.seek(0); img_reader = ImageReader(img_bytes); iw, ih = img_reader.getSize(); aspect = ih / float(iw)
...
def draw_image_cover(canvas, img_reader, x: float, y: float, w: float, h: float):
    try:
        ...
        scale = max(w / float(iw), h / float(ih)); sw, sh = float(iw) * scale, float(ih) * scale
        dx, dy = x - (sw - w) / 2.0, y - (sh - h) / 2.0
        p = canvas.beginPath(); p.rect(x, y, w, h); canvas.saveState(); canvas.clipPath(p, stroke=0, fill=0)
...
def draw_header_footer(canvas, _doc):
    canvas.saveState()
    canvas.setFont("Helvetica-Bold", 10); canvas.setFillColor(colors.black); canvas.drawString(2 * cm, A4[1] - 1.5 * cm, "Wind Blade Inspection Report - Preventive Maintenance")
    canvas.setFont("Helvetica", 9); canvas.setFillColor(colors.HexColor("#1F4E79")); canvas.drawRightString(A4[0] - 2 * cm, A4[1] - 1.5 * cm, "Arthwind Visibility and Prediction")
    canvas.setFont("Helvetica-Oblique", 8); canvas.setFillColor(colors.grey); canvas.drawRightString(A4[0] - 2 * cm, A4[1] - 1.9 * cm, "Manufacturing - Construction - Warrant - [Preventive]")
    canvas.setFont("Helvetica", 9); canvas.setFillColor(colors.black); canvas.drawCentredString(A4[0] / 2, 1 * cm, f"{canvas.getPageNumber()}")
...
def generate_pdf(results: Dict[str, Any], studs_ausentes_dict: Dict[str, List[int]], progress_callback=None):
    ...
    for idx, (blade, sensors_data) in enumerate(pdf_data):
        ...
        img_io_polar, table_data_radar = create_radar_chart_and_table(blade, latest_sensors, studs_ausentes_dict)
        ...
        for i in range(1, len(table_data_radar)): table_styles.append(('BACKGROUND', (2, i), (2, i), colors.HexColor(severity_color(table_data_radar[i][2]))))
        ...
        studs_ausentes = studs_ausentes_dict.get(str(blade), []) if isinstance(studs_ausentes_dict, dict) else []
        for stud in studs_ausentes:
            stud_data.append([str(stud), f"{calculate_angle(stud):.1f}°", get_stud_zone(stud)])
        ...
        for item in sensors_data:
            ...
            for stat in item["stats"]: table_data.append([stat["Campanha"], f"{stat['Gap (mm)']:.1f}".replace(".", ",") if pd.notna(stat['Gap (mm)']) else "-"])
...
def generate_client_pdf(results: Dict[str, Any], studs_ausentes_dict: Dict[str, List[int]], progress_callback=None):
    ...
    for item in verify_data:
        sub = item["data"].copy(); sub["Sensor"] = f"{item['casca']}-{item['regiao']}"
        sub["Blade"] = item["blade"]; sub["Inspecao"] = item["inspecao"]; flat_data.append(sub)
    ...
    for idx, blade in enumerate(results["meta"]["blades"]):
        ...
        img_io_polar, table_data = create_radar_chart_and_table(blade, latest_sensors, studs_ausentes_dict)
        rl_polar = Image(img_io_polar, width=9*cm, height=9*cm)
        ...
        for i in range(1, len(table_data)): table_styles.append(('BACKGROUND', (2, i), (2, i), colors.HexColor(severity_color(table_data[i][2]))))
        t_sev.setStyle(TableStyle(table_styles))
        ...
        story.append(Spacer(1, 0.5 * cm)); story.append(Paragraph("Missing/Broken Studs Record", h1))
        stud_data = [["ID Stud", "Ângulo (°)", "Zona Engenharia"]]
        studs_ausentes = studs_ausentes_dict.get(str(blade), []) if isinstance(studs_ausentes_dict, dict) else []
...
# --- NOVO: BOXPLOT DE FROTA (GAP VS ÁREA AFETADA) ---
        st.markdown("---")
        st.markdown("### 📦 Correlação: Gap Máximo vs. Área Afetada")
        st.info("Distribuição do maior GAP registrado em cada pá vs. a porcentagem da raiz que ultrapassou o limite de normalidade (>1.0mm).")
        
        df_box = results.get("latest_sensors", pd.DataFrame()).copy()
        if not df_box.empty:
            # 1. Calcula o Max Gap e a quantidade de relógios afetados por pá
            def calc_blade_stats(g):
                max_gap = g["Delta_medio_ciclo_mm"].max()
                # Conta quantos relógios passaram de 1.0mm
                afetados = (g["Delta_medio_ciclo_mm"] > 1.0).sum() 
                return pd.Series({"Max_Gap": max_gap, "Afetados": afetados})
                
            blade_stats = df_box.groupby(["Turbina", "Blade"]).apply(calc_blade_stats).reset_index()
            
            # 2. Categoriza a área (cada relógio = 10%)
            def categorize_area(afetados):
                pct = afetados * 10
                if pct == 0: return "0%"
                elif pct == 10: return "10%"
                elif pct == 20: return "20%"
                elif pct == 30: return "30%"
                elif pct == 40: return "40%"
                else: return "≥50%"
                
            blade_stats["Area_Afetada"] = blade_stats["Afetados"].apply(categorize_area)
            cat_order = ["0%", "10%", "20%", "30%", "40%", "≥50%"]
            
            # Conta a volumetria (n=X) para colocar no eixo
            ...
            fig_box = px.box(
                ...
                x="Categoria_Eixo", 
                y="Max_Gap", 
                points="all", # Mostra os pontinhos das pás em cima das caixas
            ...
            # Adiciona a linha de gatilho em 1.0mm
            fig_box.add_hline(y=1.0, line_dash="dash", line_color="#ff4c4c", annotation_text="1.0 mm (gatilho)")
            fig_box.update_layout(showlegend=False, height=450, margin=dict(t=30, b=40, l=40, r=40))
            
            st.plotly_chart(fig_box, use_container_width=True)
        else:
            st.warning("Dados insuficientes para gerar o Boxplot de Frota.")
...
# =========================================================================
    ...
    # =========================================================================
        ...
        st.subheader("Auditoria Técnica e Processamento de Sinal")
        
        has_calibre = "df_calibre_proc" in st.session_state and not st.session_state["df_calibre_proc"].empty
        
        if has_calibre: sub_tabs = st.tabs(["📈 Auditoria BlueDial", "🧭 Dashboard Calibre", "🎯 Cruzamento Relógio vs Calibre"])
        else:
        ...
        with sub_tabs[0]:
            ...
            if not df_eng.empty:
                ...
                with g_eng1:
                    if not df_eng_ps.empty:
                        ...
                        fig_ps.add_hline(y=5.0, line_dash="dash", line_color="red", annotation_text="Limite SEV5")
            ...
            # ----- RADAR POLIGONAL DO RELÓGIO -----
            st.markdown("### 🧭 Radar de Deslocamento Angular (BlueDial)")
            st.info("Distribuição do GAP dinâmico máximo por sensor. Limite de tolerância em 1mm (linha verde tracejada). Cada relógio acima de 1mm representa 10% de área afetada.")
            
            if not df_eng_plot.empty:
                fig_bd_radar = go.Figure()
                theta_circle = np.linspace(0, 360, 100)
                fig_bd_radar.add_trace(go.Scatterpolar(r=[1.0]*100, theta=theta_circle, mode='lines', line=dict(color='green', width=2, dash='dash'), name='Normalidade (1mm)', hoverinfo='none'))
                
                camps_bd = sorted(df_eng_plot["Inspecao"].unique())
                for camp in camps_bd:
                    d_c = df_eng_plot[df_eng_plot["Inspecao"] == camp]
                    pts, afetados = [], 0
            ...
            if plots_sinal:
                ...
                if sens_sel == "Visualizar Todos (Empilhado)":
                    # Criamos um gráfico por sensor, um embaixo do outro
                    for s in sensores_disponiveis:
                        df_p = df_sinal[df_sinal["Sensor"] == s]
                        fig = px.line(
                            df_p, x="Ponto", y="Valor_mm", color="Campanha",
                            title=f"Sinal: {s}",
                            height=300 # Altura fixa para cada "tira" de gráfico
                        )
                        fig.update_layout(margin=dict(t=40, b=20))
                        st.plotly_chart(fig, use_container_width=True)
                else:
                    # Gráfico único em destaque
                    df_p = df_sinal[df_sinal["Sensor"] == sens_sel]
                    fig = px.line(
                        df_p, x="Ponto", y="Valor_mm", color="Campanha",
                        title=f"Sinal em Destaque: {sens_sel}",
                        height=500
                    )
                    st.plotly_chart(fig, use_container_width=True)
            else:
                st.warning("Certifique-se de que há campanhas selecionadas na barra lateral.")
        ...
        if has_calibre:
            with sub_tabs[1]:
                ...
                # O NOVO FILTRO FÁCIL DO CALIBRE
                ...
                if df_cal.empty:
                    st.warning("A filtragem não retornou dados para o Calibre.")
                else:
                    def build_ticks_perimeter(perim_mm, step_mm=500.0):
                        marks = np.arange(0, perim_mm, step_mm).astype(float)
                        theta = np.mod(180.0 - (marks / perim_mm) * 360.0, 360.0)
                        return theta.tolist(), [f"{int(m)}" for m in marks]

                    def _clip_mm_for_visual(x): return np.clip(np.abs(np.asarray(x, dtype=float)), 0.0, 5.0)

                    def _series_has_valid_nonzero(y_raw, treat_as_abs):
                        y = np.asarray(y_raw, dtype=float)
                        finite = np.isfinite(y)
                        if finite.sum() < 2: return False
                        return np.nanmax(np.abs(y[finite])) > 0 if treat_as_abs else np.nanmax(y[finite]) > 0

                    def build_radar_mm(df_blade, perim_mm, y_col, campaigns, title, overlay_red=False):
                        fig = go.Figure()
                        tickvals, ticktext = build_ticks_perimeter(perim_mm)
                        treat_as_abs = (y_col in ("M3H", "M9H"))
                        added_any = False
                        for camp in campaigns:
                            d_c = df_blade[df_blade["Campaign"] == camp].dropna(subset=["theta_deg","dist_bf_mm"]).sort_values("dist_bf_mm")
                            if d_c.empty: continue
                            theta = d_c["theta_deg"].values.astype(float)
                            y_raw = pd.to_numeric(d_c[y_col], errors="coerce").values.astype(float)
                            if not _series_has_valid_nonzero(y_raw, treat_as_abs): continue
                            ...
                            if overlay_red and y_col == "GAP":
                                ...
                                if _series_has_valid_nonzero(g_raw, False):
                                    ...
                                    if np.isfinite(g_red).sum() >= 2:
                                        fig.add_trace(go.Scatterpolar(r=g_red, theta=theta, mode="lines", line=dict(width=6, color="red"), showlegend=False, name="Affected (GAP>0)"))
                        fig.update_layout(
                            title=title, margin=dict(l=40, r=40, t=80, b=30), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0),
                            polar=dict(angularaxis=dict(tickmode="array", tickvals=tickvals, ticktext=ticktext, rotation=0, direction="counterclockwise"),
                        ...
                        if not added_any: fig.update_layout(showlegend=False)
                        return fig
                    ...
                    st.markdown("---")
                    st.markdown("#### 🧭 Radar Consolidado — Frequência de Ponto Afetado")
                    fig_cons, freq_tbl = build_consolidated_frequency_radar(df_cal, float(perim_mm), float(bin_mm))
                    st.plotly_chart(fig_cons, use_container_width=True)

                    st.markdown("---")
                    st.markdown("#### 📌 Pontos mais críticos (Top N) — 1 ponto por pá")
                    
                    def _pick_worst_per_blade(df_in, score_series):
                        tmp = df_in.copy()
                        tmp["_score"] = pd.to_numeric(score_series, errors="coerce")
                        tmp = tmp.dropna(subset=["_score"])
                        if tmp.empty: return tmp.drop(columns=["_score"], errors="ignore")
                    ...
                    for tb, bl in cal_blades_list[:20]:
                        ...
                        camps_case = sorted(d_bl["Campaign"].unique().tolist())
                        
                        cA, cB, cC = st.columns(3)
                        with cA:
                            fig3 = build_radar_mm(d_bl, float(perim_mm), "M3H", camps_case, "3h (mm)", False)
                            st.plotly_chart(fig3, use_container_width=True, key=f"cal_3h_{tb}_{bl}")
                        with cB:
            ...
            with sub_tabs[2]:
                ...
                st.info("Sobreposição do GAP dinâmico (Relógio) e estático (Calibre) ao longo da circunferência da raiz da pá.")
                
                df_cal_raw = st.session_state.get("df_calibre_proc", pd.DataFrame())
                if not df_cal_raw.empty:
                    ...
                    cal_max = df_cal_raw.groupby(["Turbina", "Blade"], as_index=False).agg(Max_Gap_Calibre=("GAP", "max"))
                    
                    bd_max["Turbina"] = bd_max["Turbina"].astype(str).str.strip()
                    bd_max["Blade"] = bd_max["Blade"].astype(str).str.strip()
                    cal_max["Turbina"] = cal_max["Turbina"].astype(str).str.strip()
                    cal_max["Blade"] = cal_max["Blade"].astype(str).str.strip()
                    ...
                    with st.expander("Tabela de Diferencial de Severidade (Expandir)"):
                        st.dataframe(cross.style.format({col: "{:.2f}" for col in ["Max_Gap_BlueDial", "Max_Gap_Calibre", "Diferença (mm)"]}).background_gradient(subset=["Diferença (mm)"], cmap="Reds"), use_container_width=True)

                    st.divider()
                    st.markdown("#### Radar Comparativo de Curvas de Gap")
                    
                    c_cruz1, c_cruz2 = st.columns(2)
                    with c_cruz1: 
                        cross_turb = st.selectbox("Selecione a Turbina:", sorted(cross["Turbina"].unique()), key="cross_t")
                    with c_cruz2:
                        cross_blades = sorted(cross[cross["Turbina"] == cross_turb]["Blade"].unique())
                        cross_blade = st.selectbox("Selecione a Pá:", cross_blades, key="cross_b")
                    ...
                    df_cal_blade = df_cal_raw[(df_cal_raw["Turbina"].astype(str) == cross_turb) & (df_cal_raw["Blade"].astype(str) == cross_blade)].copy()
                    ...
                    if not df_bd_blade.empty or not df_cal_blade.empty:
                        ...
                        if not df_bd_blade.empty:
                            bd_points = []
                            for sens_key in MAPA_FUROS.keys():
                                row = df_bd_blade[(df_bd_blade["Casca"] + "-" + df_bd_blade["Regiao"]) == sens_key]
                                gap = row["Delta_medio_ciclo_mm"].max() if not row.empty else np.nan
                                if not pd.isna(gap):
                                    ang = calculate_angle(MAPA_FUROS[sens_key])
                                    bd_points.append({"angulo": ang, "gap": gap, "sensor": sens_key})
                            
                            if bd_points:
                                df_bd_pts = pd.DataFrame(bd_points).sort_values("angulo")
                                max_bd = df_bd_pts["gap"].max()
                                ...
                                fig_cross.add_trace(go.Scatterpolar(
                                    ...
                                    marker=dict(size=10, color='blue', symbol='square'), line=dict(color='blue', width=2),
                                    name='Relógio (Dinâmico)', hoverinfo='text',
                                    hovertext=[f"<b>{s}</b><br>Gap Dinâmico: {g:.2f}mm" for s, g in zip(df_bd_pts["sensor"], df_bd_pts["gap"])]
                                ))

                        max_cal = 0
                        if not df_cal_blade.empty:
                            df_cal_blade = df_cal_blade.sort_values("theta_deg")
                            max_cal = df_cal_blade["GAP"].max()
                            
                            if not df_cal_blade.empty:
                                df_cal_cycle = pd.concat([df_cal_blade, df_cal_blade.iloc[[0]]], ignore_index=True)
                                fig_cross.add_trace(go.Scatterpolar(
                                    r=df_cal_cycle["GAP"], theta=df_cal_cycle["theta_deg"], mode='lines+markers',
                                    marker=dict(size=6, color='red'), line=dict(color='red', width=2, dash='dash'),
                                    name='Calibre (Estático)', hoverinfo='text',
                                    hovertext=[f"Gap Estático: {g:.2f}mm<br>Ângulo: {a:.1f}°" for g, a in zip(df_cal_cycle["GAP"], df_cal_cycle["theta_deg"])]
                                ))
                                
                        max_r = max(max_bd, max_cal) * 1.2
                        if pd.isna(max_r) or max_r < 1: max_r = 5.0

                        # INJEÇÃO DOS STUDS AUSENTES NO RADAR COMPARATIVO
                        studs_ausentes = studs_ausentes_dict.get(cross_blade, []) if isinstance(studs_ausentes_dict, dict) else []
                        for stud in studs_ausentes:
                            ang = calculate_angle(stud)
                            # Posiciona os studs na borda externa do gráfico (max_r)
                            fig_cross.add_trace(go.Scatterpolar(
                                r=[max_r * 0.98], theta=[ang], mode='markers',
                        ...
                        fig_cross.update_layout(
                            polar=dict(
                                angularaxis=dict(direction="counterclockwise", rotation=0, tickmode='array', tickvals=[0, 90, 180, 270], ticktext=["LE", "PS", "TE", "SS"]),
                                radialaxis=dict(showticklabels=True, range=[0, max_r])
                            ),
                            title=f"Sobreposição Dimensional: Pá {cross_blade}", height=550, margin=dict(l=40, r=40, t=60, b=40),
                        ...
                        st.divider()
                        st.markdown("#### 📋 Detalhamento das Leituras (Pá Selecionada)")
                        ...
                        with col_raw_bd:
                            ...
                            if not df_bd_blade.empty:
                                ...
                                df_bd_show = df_bd_show.groupby("Sensor").agg(
                                    Gap_mm=("Delta_medio_ciclo_mm", "max")
                                ).reset_index()
                                df_bd_show["Severity"] = df_bd_show["Gap_mm"].apply(classify_severity)
                                
                                st.dataframe(
                                    df_bd_show.style.format({"Gap_mm": "{:.2f}"})
                                    .apply(lambda x: [f'background-color: {severity_color(v)}' if x.name == 'Severity' else '' for v in x], axis=0),
                                    use_container_width=True, hide_index=True
                                )
                            else:
                                st.warning("Sem dados de relógio para esta pá.")
                        ...
                        with col_raw_cal:
                            ...
                            if not df_cal_blade.empty:
                                ...
                                st.dataframe(
                                    ...
                                    .style.format({"Gap (mm)": "{:.2f}", "Distância (mm)": "{:.0f}"}),
                                    use_container_width=True, hide_index=True
                                )
                            else:
                                st.warning("Sem dados de calibre para esta pá.")
                        # --- FIM DO BLOCO NOVO ---
    ...
    # =========================================================================
        ...
        for blade in blades_sel[:max_plots_view]:
            ...
            if not df_viz_cli.empty:
                ...
                with col_ps:
                    if plots_ps:
                        ...
                        fig_ps = px.line(g_plot_ps, x="Ponto", y="Valor_mm", color="Sensor", height=350, title=f"PS Curves - {data_str}")
                        st.plotly_chart(fig_ps, use_container_width=True, key=f"cli_line_ps_{blade}")

                with col_ss:
                    if plots_ss:
                        g_plot_ss = pd.concat(plots_ss, ignore_index=True)
                        fig_ss = px.line(g_plot_ss, x="Ponto", y="Valor_mm", color="Sensor", height=350, title=f"SS Curves - {data_str}")
            ...
            for sens_key in MAPA_FUROS.keys():
                ...
                if not pd.isna(gap):
                    sev = classify_severity(gap)
                    radar_data.append({"Sensor": sens_key, "Gap_mm": gap, "Severidade": sev})
            
            with col_radar:
                fig_radar = create_radar_chart(blade, latest_sensors, studs_ausentes_dict, engine='plotly')
            ...
            with col_table:
                ...
                if not df_radar.empty:
                    ...
                    st.dataframe(df_radar.style.format({"Gap_mm": "{:.1f}"}), hide_index=True, use_container_width=True)
