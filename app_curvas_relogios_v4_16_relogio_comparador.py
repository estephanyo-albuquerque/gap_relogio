...
# =====================================================================
# CLASSIFICAÇÃO DE SEVERIDADE — DUAS FUNÇÕES, UMA POR PADRÃO
# =====================================================================
...
def severity_recommendation(sev: str, modelo: str = "Arthwind") -> Tuple[str, dt.timedelta]:
    """Retorna (texto_recomendação, timedelta) correto para cada padrão."""
    if modelo == "ENEL":
        recs = {
...
def run_analysis(df_in: pd.DataFrame, full_process=False, t_sel=None, b_sel=None, i_sel=None, modelo="Arthwind"):
    """
    Motor de análise. Recebe 'modelo' para classificar severidades corretamente
    em TODA a pipeline (tabelas, radar, PDF).
    """
    ...
    reinspection = delta_summary.groupby(["Turbina", "Blade"], as_index=False).agg(
        Reinspections=("Inspecao", "nunique"),
        First_Date=("Data", "min"),
        Last_Date=("Data", "max")
    )
    ...
    return {
        "meta": {
            ...
            "date": cover_date, "campaign_dates": campaign_dates_str,
            "modelo": modelo   # propagado para uso no PDF
        },
        ...
        "classify_fn": _classify   # função correta passada junto ao resultado
    }
...
def create_radar_chart_and_table(blade, latest_sensors, studs_ausentes_dict, classify_fn=None):
    ...
    sorted_sensors['Furo'] = sorted_sensors.apply(
        lambda row: MAPA_FUROS.get(f"{row['Casca']}-{row['Regiao']}"), axis=1
    )
    ...
    for _, row in sorted_sensors.iterrows():
        ...
        sev = classify_fn(val_gap)      # <-- usa a função correta
        ...
        table_data.append([
            sens_key,
            f"{val_gap:.1f}".replace(".", ",") if pd.notna(val_gap) else "-",
            sev
        ])
...
def _create_cover_and_intro(doc, results, h1, normal, modelo="Arthwind", windfarm=None, customer=None):
    ...
    def draw_cover_full(canvas, _doc):
        ...
        if logo_bytes:
            try:
                canvas.drawImage(
                    ...
                    width=COVER_LOGO_W_CM * cm, height=COVER_LOGO_H_CM * cm,
        ...
        for i, lab in enumerate(labels):
            canvas.drawString(COVER_META_LABEL_X_CM * cm, y0 - i * line_h, lab)
        ...
        for i, val in enumerate(values):
            yy = y0 - i * line_h
            if i == 2:
                _draw_wrapped(canvas, str(val), COVER_META_VALUE_X_CM * cm, yy, max_chars=65, line_h_pt=10)
            else:
                canvas.drawString(COVER_META_VALUE_X_CM * cm, yy, str(val))
        canvas.restoreState()
    ...
    story.append(Paragraph(
        ...
        f"serial numbers {blades_list_txt}, installed on the {turbina_txt} wind turbine located at the {windfarm_txt}.",
        normal
    ))
    ...
    if sev_df is not None and not sev_df.empty:
        ...
        for _, row in sev_df.iterrows():
            ...
            # Reclassifica com a função correta ao montar a tabela de conclusão
        ...
        # Recomendação final baseada no pior SEV, usando o padrão correto
        if modelo == "ENEL":
            recs_list = ["6 Months", "6 Months", "3 Months", "1 Month", "15 Days", "STOP WTG"]
        else:
            recs_list = ["12 Months", "6 Months", "3 Months", "1 Month", "15 Days", "Stop Turbine"]
    ...
    story.append(Paragraph(
        "The operation developed by Arthwind consists on inspecting the root gap of wind turbine blades "
        "using dial indicators, with the equipment installed externally around the blade root.", normal
    ))
    if load_image_for_pdf("METOD_ROTOR"):
        story.append(Table(
            [[get_proportional_image(load_image_for_pdf("METOD_ROTOR"), max_w=12*cm, max_h=5.5*cm)]],
            colWidths=[A4[0] - doc.leftMargin - doc.rightMargin],
            style=[('ALIGN', (0, 0), (-1, -1), 'CENTER')]
        ))
    story.append(Spacer(1, 0.5 * cm))
    story.append(Paragraph(
        "Dial indicators are mounted at specific points around the blade root using suction bases and rods. "
        "The blade is rotated 360°while the dial indicators remain in position.", normal
    ))
    if load_image_for_pdf("METOD_MAPA"):
        story.append(Table(
            [[get_proportional_image(load_image_for_pdf("METOD_MAPA"), max_w=12*cm, max_h=5.5*cm)]],
            colWidths=[A4[0] - doc.leftMargin - doc.rightMargin],
            style=[('ALIGN', (0, 0), (-1, -1), 'CENTER')]
        ))
    story.append(Spacer(1, 0.5 * cm))
    story.append(Paragraph(
        "After the rotation, the maximum and minimum displacement values are analyzed to check the total "
        "variation at each point. This procedure is repeated for all blades on the turbine.", normal
    ))
    if load_image_for_pdf("METOD_BASE"):
        story.append(Table(
            [[get_proportional_image(load_image_for_pdf("METOD_BASE"), max_w=12*cm, max_h=5.5*cm)]],
            colWidths=[A4[0] - doc.leftMargin - doc.rightMargin],
            style=[('ALIGN', (0, 0), (-1, -1), 'CENTER')]
        ))
    story.append(PageBreak())
    ...
    story.append(Paragraph(
        ...
        "The scope encompasses the analysis of displacement data collected via dial indicators during a full "
        "rotor rotation. The primary objective is to evaluate the gap variation at multiple specific points "
        "around the circumference (PS, SS, LE, TE), classify the severity of any deviations according to "
        "the client standards, and provide actionable maintenance recommendations to ensure the structural "
        "integrity and safe operation of the equipment.", normal
    ))
    story.append(Spacer(1, 0.5 * cm))
...
def draw_header_footer(canvas, _doc):
    ...
    if load_image_for_pdf("LOGO"):
        try:
            canvas.drawImage(
                ImageReader(load_image_for_pdf("LOGO")),
                A4[0] - 5 * cm, 0.8 * cm,
...
def generate_pdf(results: Dict[str, Any], studs_ausentes_dict: Dict[str, List[int]], progress_callback=None, modelo="Arthwind", windfarm=None, customer=None):
    ...
    for idx, (blade, sensors_data) in enumerate(pdf_data):
        ...
        for i in range(1, len(table_data_radar)):
            table_styles.append(('BACKGROUND', (2, i), (2, i), colors.HexColor(severity_color(table_data_radar[i][2]))))
        ...
        if len(stud_data) > 1:
            ...
            story.append(Paragraph("Nenhum stud estrutural ausente reportado para esta pá.", normal))
        ...
        for item in sensors_data:
            img_io = io.BytesIO()
            item["fig"].savefig(img_io, format='png', dpi=100, bbox_inches='tight')
            img_io.seek(0)
            ...
            for stat in item["stats"]:
                table_data.append([
                    stat["Campanha"],
                    f"{stat['Gap (mm)']:.1f}".replace(".", ",") if pd.notna(stat['Gap (mm)']) else "-"
                ])
...
def generate_client_pdf(results: Dict[str, Any], studs_ausentes_dict: Dict[str, List[int]], progress_callback=None, modelo="Arthwind", windfarm=None, customer=None):
    ...
    for idx, blade in enumerate(results["meta"]["blades"]):
        ...
        if not df_viz_cli.empty:
            ...
            dt_insp = results["delta_summary"][
                (results["delta_summary"]["Blade"].astype(str) == str(blade)) &
                (results["delta_summary"]["Inspecao"] == insp_max)
            ]["Data"].max()
        ...
        for i in range(1, len(table_data)):
            table_styles.append(('BACKGROUND', (2, i), (2, i), colors.HexColor(severity_color(table_data[i][2]))))
        t_sev.setStyle(TableStyle(table_styles))
        ...
        if len(stud_data) > 1:
            ...
            story.append(Paragraph("No missing studs reported for this blade.", normal))
...
def generate_excel_report(delta_summary: pd.DataFrame):
    ...
    pivot = df_pivot.pivot_table(
        ...
        columns='Sensor_Key', values='Delta_medio_ciclo_mm', aggfunc='first'
    )
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        for c_idx, val in enumerate(pivot.columns.values):
        ...
        for r in range(max_r):
            ...
            for c_idx in range(max_c):
                ...
                ws.write(
                    r + 1, c_idx,
                    val if c_idx == col_sev else ("" if pd.isna(val) else val),
                    sev_fmts.get(sev_val, fmt_border) if c_idx == col_sev else fmt_border
                )
    output.seek(0)
...
if "results" in st.session_state and st.session_state["results"] is not None:
    ...
    if aba_selecionada == "📊 Resumo Executivo":
        ...
        if not df_box.empty:
            def calc_blade_stats(g):
                ...
                afetados = (g["Delta_medio_ciclo_mm"] > limiar_area).sum()
            ...
            fig_box = px.box(
                blade_stats, x="Categoria_Eixo", y="Max_Gap", points="all",
        ...
        with sub_tabs[0]:
            ...
            if not df_eng.empty:
                ...
                with g_eng1:
                    if not df_eng_ps.empty:
                        ...
                        fig_ps.add_hline(y=limite_sev5, line_dash="dash", line_color="red",
                                         annotation_text=f"Limite crítico ({limite_sev5}mm)")
                ...
                with g_eng2:
                    if not df_eng_ss.empty:
                        ...
                        fig_ss.add_hline(y=limite_sev5, line_dash="dash", line_color="red",
                                         annotation_text=f"Limite crítico ({limite_sev5}mm)")
            ...
            if not df_eng_plot.empty:
                ...
                fig_bd_radar.add_trace(go.Scatterpolar(
                    r=[limiar_radar]*100, theta=theta_circle, mode='lines',
                    line=dict(color='green', width=2, dash='dash'),
                    name=f'Normalidade ({limiar_radar}mm)', hoverinfo='none'
                ))
            ...
            if plots_sinal:
                ...
                if sens_sel == "Visualizar Todos (Empilhado)":
                    ...
                    fig = px.line(df_p, x="Ponto", y="Valor_mm", color="Campanha",
                                  title=f"Sinal em Destaque: {sens_sel}", height=500)
                    st.plotly_chart(fig, use_container_width=True)
            else:
        ...
        if has_calibre:
            with sub_tabs[1]:
                ...
                if df_cal.empty:
                    ...
                    def build_ticks_perimeter(perim_mm_val, step_mm=500.0):
                        marks = np.arange(0, perim_mm_val, step_mm).astype(float)
                        theta = np.mod(180.0 - (marks / perim_mm_val) * 360.0, 360.0)
                    ...
                    def _series_has_valid_nonzero(y_raw, treat_as_abs):
                        y = np.asarray(y_raw, dtype=float); finite = np.isfinite(y)
                        if finite.sum() < 2: return False
                        return np.nanmax(np.abs(y[finite])) > 0 if treat_as_abs else np.nanmax(y[finite]) > 0

                    def build_radar_mm(df_blade, perim_mm_val, y_col, campaigns, title, overlay_red=False):
                        fig = go.Figure()
                        tickvals, ticktext = build_ticks_perimeter(perim_mm_val)
                            ...
                            if overlay_red and y_col == "Gap (mm)":
                                ...
                                if _series_has_valid_nonzero(g_raw, False):
                                    ...
                                    if np.isfinite(g_red).sum() >= 2:
                                        fig.add_trace(go.Scatterpolar(
                                            r=g_red, theta=theta, mode="lines",
                                            line=dict(width=6, color="red"),
                                            showlegend=False, name="Affected (GAP>0)"
                                        ))
                        fig.update_layout(
                            title=title, margin=dict(l=40, r=40, t=80, b=30),
                    ...
                    def build_consolidated_frequency_radar(df_sel, perim_mm_val, bin_mm_val):
                        d = df_sel.copy()
                        dist_col = "dist_bf_mm"; gap_col = "Gap (mm)"
                        d["bin_mm"] = (np.round(pd.to_numeric(d[dist_col], errors="coerce") / bin_mm_val) * bin_mm_val).clip(0.0, perim_mm_val)
                        ...
                        tickvals, ticktext = build_ticks_perimeter(perim_mm_val)
                        theta = np.mod(180.0 - (freq["bin_mm"].values.astype(float) / perim_mm_val) * 360.0, 360.0)
                        r = pd.to_numeric(freq["Blades_Affected"], errors="coerce").fillna(0).values.astype(float)
                        fig = go.Figure()
                        fig.add_trace(go.Barpolar(
                            r=r, theta=theta, width=360.0 * (bin_mm_val / perim_mm_val),
                        ...
                        fig.update_layout(
                            ...
                            margin=dict(l=40, r=40, t=80, b=30), showlegend=False,
                            polar=dict(
                                angularaxis=dict(tickmode="array", tickvals=tickvals, ticktext=ticktext, rotation=0, direction="counterclockwise"),
            ...
            with sub_tabs[2]:
                ...
                if not df_cal_raw.empty:
                    ...
                    for df_m in [bd_max, cal_max]:
                        df_m["Turbina"] = df_m["Turbina"].astype(str).str.strip()
                        df_m["Blade"] = df_m["Blade"].astype(str).str.strip()
                    ...
                    with st.expander("Tabela de Diferencial de Severidade (Expandir)"):
                        st.dataframe(
                            cross.style.format({col: "{:.2f}" for col in ["Max_Gap_BlueDial", "Max_Gap_Calibre", "Diferença (mm)"]}).background_gradient(subset=["Diferença (mm)"], cmap="Reds"),
                    ...
                    df_bd_blade = results["latest_sensors"][
                        (results["latest_sensors"]["Turbina"].astype(str) == cross_turb) &
                        (results["latest_sensors"]["Blade"].astype(str) == cross_blade)
                    ].copy()
                    df_cal_blade = df_cal_raw[
                        (df_cal_raw["Turbina"].astype(str) == cross_turb) &
                        (df_cal_raw["Blade"].astype(str) == cross_blade)
                    ].copy()
                    ...
                    if not df_bd_blade.empty or not df_cal_blade.empty:
                        ...
                        if not df_bd_blade.empty:
                            ...
                            for sens_key in MAPA_FUROS.keys():
                                ...
                                gap = row["Delta_medio_ciclo_mm"].max() if not row.empty else np.nan
                                if not pd.isna(gap):
                                    bd_points.append({"angulo": calculate_angle(MAPA_FUROS[sens_key]), "gap": gap, "sensor": sens_key})
                            if bd_points:
                                ...
                                fig_cross.add_trace(go.Scatterpolar(
                                    r=df_bd_pts["gap"], theta=df_bd_pts["angulo"],
                        ...
                        if not df_cal_blade.empty:
                            ...
                            fig_cross.add_trace(go.Scatterpolar(
                                r=df_cal_cycle["Gap (mm)"], theta=df_cal_cycle["theta_deg"],
                                mode='lines+markers',
                                marker=dict(size=6, color='red'), line=dict(color='red', width=2, dash='dash'),
                                name='Calibre (Estático)',
                        ...
                        with col_raw_cal:
                            ...
                            if not df_cal_blade.empty:
                                ...
                                df_cal_show["Casca"] = df_cal_show["theta_deg"].apply(lambda ang: "PS" if 0 <= ang <= 180 else "SS")
                                df_cal_show = df_cal_show[["Casca", "Bordo Ref.", "Distância (mm)", "Gap (mm)"]]
                                st.dataframe(
                                    df_cal_show.sort_values(by=["Casca", "Distância (mm)"]).style.format({"Gap (mm)": "{:.2f}", "Distância (mm)": "{:.0f}"}),
                                    use_container_width=True, hide_index=True
                                )
                            else:
    ...
    # =========================================================================
        ...
        st.subheader(f"🎯 Dashboard Executivo Simplificado (Por Pá) — Padrão: {modelo_atual}")
        ...
        for blade in blades_sel[:max_plots_view]:
            ...
            for sens_key in MAPA_FUROS.keys():
                ...
                gap = row["Delta_medio_ciclo_mm"].max() if not row.empty else np.nan
                if not pd.isna(gap):
                    sev = _classify_dash(gap)
                    radar_data.append({"Sensor": sens_key, "Gap_mm": gap, "Severidade": sev})

            with col_radar:
                fig_radar = create_radar_chart(blade, latest_sensors, studs_ausentes_dict, engine='plotly', classify_fn=_classify_dash)
    ...
    # =========================================================================
        ...
        # Modelo já foi definido no cálculo — mostra o que está ativo
                ...
                if turbs_to_run:
                    ...
                    with zipfile.ZipFile(zip_buffer, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
                        for i, tb in enumerate(turbs_to_run, start=1):
                            ...
                            i_sel_zip = sorted(df_tb["Inspecao"].dropna().unique().tolist())
                    ...
                    my_bar_zip_eng.progress(1.0, text=f"✅ ZIP ({modelo_ativo}) concluído!")
                    zip_buffer.seek(0)
                    st.download_button(
                        f"📥 Baixar ZIP ({modelo_ativo})",
                        data=zip_buffer.getvalue(),
                        file_name=f"Relatorios_Engenharia_{modelo_ativo}.zip",
                        mime="application/zip"
                    )
                    if errors:
                        st.warning("Falhas:")
                        st.dataframe(pd.DataFrame(errors, columns=["Turbina", "Erro"]))
        ...
        with col_down_i:
            down_insp = st.selectbox(
                "Selecione a Campanha / Data:",
                sorted(df_raw[df_raw["Turbina"] == down_turb]["Inspecao"].dropna().unique()),
                key="down_i"
            )
        with col_down_btn:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("📄 Processar PDF"):
                with st.spinner(f"Gerando Relatório {modelo_ativo}..."):
                    b_sel_cli = sorted(
                        df_raw[(df_raw["Turbina"] == down_turb) & (df_raw["Inspecao"] == down_insp)]["SN_da_Pa"].dropna().unique().tolist()
                    )
                    if b_sel_cli:
                        try:
